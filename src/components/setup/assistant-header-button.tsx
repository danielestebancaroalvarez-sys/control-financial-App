...
export function AssistantHeaderButton({ state }: { state: AssistantState }) {
  ...
  const moduleAccent = MODULE_ACCENTS[appModule]
  const nextStep = moduleState.steps.find(s => !s.completed && !s.optional)
  const dotAccent = nextStep
    ? getGuideStepTheme(nextStep.id).accent
    : moduleAccent
  ...
  return (
    <>
      ...
      >
        <Sparkles className="w-5 h-5" style={{ color: moduleAccent }} />
        {showDot && (
          <span
