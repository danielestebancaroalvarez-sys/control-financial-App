...
export type ResolvedGuideStepTheme = GuideStepTheme & {
  surfaceBg: string
}
...
export function resolveGuideStepTheme(
  stepId: string,
  isDark: boolean
): ResolvedGuideStepTheme {
  const theme = getGuideStepTheme(stepId)
  return {
    ...theme,
...
export const SAVINGS_ACCENT = GUIDE_STEP_THEMES.savings.accent
